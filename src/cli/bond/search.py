...
def print_hk_fund_search_results(results: list, show_all: bool = False):
    """打印香港基金搜索结果"""
    if not results:
        print("  ℹ️  未找到匹配的香港基金")
        return
    ...
    print(f"  查找到 {count} 只香港基金 (显示前 {display_count} 只):")
    print()
    print(f"  {'序号':<6}{'基金代码':<12}{'基金简称':<25}{'近1年':<12}")
    print("  " + "-" * 55)

    for i, item in enumerate(results[:display_count], 1):
        code = str(item.get("基金代码", ""))
        name = str(item.get("基金简称", ""))[:23]
        v = item.get("近1年")
        v_str = f"{v:.2f}" if v is not None else "N/A"
        print(f"  {i:<6}{code:<12}{name:<25}{v_str:<12}")
...
def search(
    ...
    hk: bool = typer.Option(False, "--hk", help="搜索香港基金"),
):
    """搜索基金"""
    print_banner()
    ...
    if hk:
        ...
        if result.get("status") == "error":
            ...
            print_hk_fund_search_results(result.get("data", []), show_all=show_all)
    else:
